...
export class MintActions {
  ...
  async dispatch(ref, ctx) {
    ...
    // Merge loop context (repeater row item) so conditions/config resolve $item.x.
    const loopCtx = ctx && ctx.loopCtx;
    const condCtx = loopCtx ? { ...this._state.getContext(), ...loopCtx } : this._state.getContext();

    if (schema.condition) {
      try { if (!evalExpr(schema.condition, condCtx)) return; } catch {}
    }

    const handler = this._handlers[schema.type];
    if (!handler) return undefined;

    const config = this._resolveConfig(schema.config || {}, loopCtx);
    const fullCtx = { state: this._state, ...ctx, loopCtx: loopCtx };
  ...
  _resolveConfig(config, loopCtx) {
    const ctx = loopCtx ? { ...this._state.getContext(), ...loopCtx } : this._state.getContext();
    const resolved = {};
  ...
  _registerBuiltins() {
    ...
    // against state + loop context (top-level config resolution only handles $strings).
    const _resolveMap = (m, loopCtx) => {
      const out = {}; const ctx = loopCtx ? { ...this._state.getContext(), ...loopCtx } : this._state.getContext();
      for (const k in (m || {})) {
        const v = m[k];
    ...
    this.register("dbInsert", async (c, ctx) => {
      const lc = ctx && ctx.loopCtx;
      const values = _resolveMap(c.values, lc); const cols = Object.keys(values);
    ...
    this.register("dbUpdate", async (c, ctx) => {
      const lc = ctx && ctx.loopCtx;
      const values = _resolveMap(c.values, lc); const where = _resolveMap(c.where, lc);
    ...
    this.register("dbDelete", async (c, ctx) => {
      const lc = ctx && ctx.loopCtx;
      const where = _resolveMap(c.where, lc); const wcols = Object.keys(where);
