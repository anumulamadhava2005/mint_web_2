...
export function generateMintRuntimeProvider(
  ...
  L("}");
  L("");
  L("function handleOnSuccess(expr: string, result: any, setState: (k: string, v: any) => void, stateRef: React.MutableRefObject<any>, allActions: Record<string, (...args: any[]) => void>) {");
  L('  if (!expr) return;');
  L('  const lines = expr.split(";").map(l => l.trim()).filter(Boolean);');
  L('  for (const line of lines) {');
  L('    const setMatch = line.match(/^setState\\(\\"([^\\"]+)\\",\\s*(.+)\\)$/);');
  L('    if (setMatch) {');
  L('      let val: any = setMatch[2];');
  L('      if (val === "$result") val = result;');
  L('      else if (val === "$result.rows") val = result?.rows;');
  L(`      else if (val === '""' || val === "''") val = "";`);
  L('      else if (val === "[]") val = [];');
  L('      else if (val === "true") val = true;');
  L('      else if (val === "false") val = false;');
  L('      else if (!isNaN(Number(val))) val = Number(val);');
  L('      setState(setMatch[1], val);');
  L('      continue;');
  L('    }');
  L('    const callMatch = line.match(/^CALL\\s+(\\w+)/);');
  ...
  L("}");
  L("");
  L("function handleAlso(expr: string, allActions: Record<string, (...args: any[]) => void>) {");
  L('  if (!expr) return;');
  L('  const callMatch = expr.match(/^CALL\\s+(\\w+)/);');
  L('  if (callMatch && allActions[callMatch[1]]) {');
  L('    setTimeout(() => allActions[callMatch[1]](), 0);');
  L('  }');
  L("}");
  L("");
