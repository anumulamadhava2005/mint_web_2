...
async function main() {
  ...
  if (!fileRes.rows.length) {
    throw new Error("CRM file not found in files table");
  }
  ...
  for (const [id, obj] of Object.entries(objects)) {
    if (obj.type === "text" && obj.content?.children) {
      for (const para of obj.content.children) {
        if (para.children) {
  ...
  function setNavColor(shape, isActive) {
    if (!shape) return;
  ...
  function buildSidebar(screenId, xOffset, activeTab) {
  ...
  function buildHeader(screenId, xOffset, title) {
  ...
  existingScreens.forEach(scInfo => {
    const screen = objects[scInfo.id];
    if (!screen) return;
    const oldSbId = screen.shapes.find(id => objects[id]?.name === "Sidebar");
    
    // Replace with completely fresh and updated sidebar
    const newSbId = buildSidebar(scInfo.id, screen.x, scInfo.tab);
    screen.shapes = screen.shapes.filter(id => id !== oldSbId);
    screen.shapes.unshift(newSbId);
  });

  // 4. Dashboard Updates: Activity Feed resize + AI Agent Terminal
  const dashScreen = objects["786f0946-18f0-40ba-a952-313bdd374c50"];
  const feedId = dashScreen.shapes.find(id => objects[id]?.name === "Activity Feed" || objects[id]?.name === "Activity");
  const feed = objects[feedId];
  if (feed) {
    feed.width = 720;
    const itemId = feed.shapes.find(id => objects[id]?.name === "Activity Item" || objects[id]?.name === "Act1");
    const item = objects[itemId];
    if (item) {
      item.width = 680;
      const txtId = item.shapes.find(id => objects[id]?.name === "Detail Text");
      const txt = objects[txtId];
  ...
  const contactsScreen = objects["6b5c3518-8e94-4a19-93dd-f63b42642137"];
  
  // Set row background onClick to select active contact
  for (const [id, obj] of Object.entries(objects)) {
    if (obj.parentId === "23a49c25-8ace-45eb-be2b-83809908609f" && obj.name.startsWith("Row ")) {
      obj.runtimeBindings = {
        ...obj.runtimeBindings,
  ...
  drawer.strokes = [{ strokeColor: "#334155", strokeWidth: 1, strokeAlignment: "CENTER" }];
  ...
  pipelineCols.forEach(col => {
    const colShape = objects[col.id];
    if (!colShape) return;
    const repeaterId = colShape.shapes.find(id => objects[id]?.name === "Card Repeater");
    const repeater = objects[repeaterId];
    if (!repeater) return;

    repeater.height = 116;
    const bgId = repeater.shapes.find(id => objects[id]?.name === "Card Bg");
    const bg = objects[bgId];
    if (bg) bg.height = 116;
  ...
  root.shapes.push(campScreenId, tickScreenId, loginScreenId, signupScreenId);

  // 11. Write back file data
  await client.query("UPDATE files SET data = $1 WHERE id = $2", [JSON.stringify(fileData), fileRow.id]);
  console.log("Updated files table successfully.");
  ...
  console.log("Updated runtime_schemas table successfully.");
  ...
  console.log("Compiled nodes count:", nodes.length);
  ...
  if (userRes.rows.length === 0) {
    ...
    console.log("Created user manimadhava43@gmail.com");
  } else {
  ...
  console.log("Invoking POST http://localhost:3001/api/commit...");
  const body = {
    projectId: PID,
    fileId: fileRow.id,
  ...
  if (response.status !== 201) {
    throw new Error(`Commit failed: ${JSON.stringify(resData)}`);
  }
  console.log("🎉 Seed and commit execution completed successfully!");
}
