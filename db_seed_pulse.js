...
function buildWorkoutsScreen(objects) {
  ...
  header.layoutProps = { layout: "flex", layoutFlexDir: "row", layoutGap: 0,
...
function buildWorkoutDetailScreen(objects) {
  ...
  header.layoutProps = { layout: "flex", layoutFlexDir: "row", layoutGap: 12,
  ...
  meta.rx = 12;
  meta.layoutProps = { layout: "flex", layoutFlexDir: "row", layoutGap: 24,
...
function buildProgressScreen(objects) {
  ...
  streakCard.rx = 12;
  streakCard.layoutProps = { layout: "flex", layoutFlexDir: "row", layoutGap: 12,
...
async function main() {
  ...
  schema.globalActions = [
    ...
    { id: "act-fetch-workouts", name: "fetchWorkouts", type: "fetch", config: {
      ...
      body: { sql: "SELECT id, name, description, duration_minutes, calories_burned, muscle_group, created_at FROM workouts WHERE user_id = $1 ORDER BY created_at DESC",
              params: ["$user.id"] },
      onSuccess: "SET $local.workouts = $result.rows"
    }},
    { id: "act-fetch-recent", name: "fetchRecentWorkouts", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "SELECT id, name, description, duration_minutes, calories_burned, muscle_group, created_at FROM workouts WHERE user_id = $1 ORDER BY created_at DESC LIMIT 5",
              params: ["$user.id"] },
      onSuccess: "SET $local.recentWorkouts = $result.rows"
    }},
    { id: "act-create-workout", name: "createWorkout", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "INSERT INTO workouts (name, description, user_id) VALUES ($1, $2, $3) RETURNING *",
    ...
    { id: "act-fetch-detail", name: "fetchWorkoutDetail", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT we.*, e.name, e.muscle_group,
    ...
    { id: "act-start-workout",    name: "startWorkout", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "UPDATE workouts SET started_at = now() WHERE id = $1 RETURNING *",
              params: ["$local.activeWorkout.id"] },
      onSuccess: "SET $local.activeWorkout = $result.rows[0]"
    }},
    { id: "act-complete-workout", name: "completeWorkout", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "UPDATE workouts SET ended_at = now(), calories_burned = $2 WHERE id = $1 RETURNING *",
    ...
    { id: "act-fetch-goals", name: "fetchGoals", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT *, ROUND((current_value::numeric / NULLIF(target_value::numeric, 0)) * 100) AS pct_complete
    ...
    { id: "act-create-goal", name: "createGoal", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "INSERT INTO goals (user_id, title, target_value, deadline, status) VALUES ($1, $2, $3, $4, 'active') RETURNING *",
    ...
    { id: "act-fetch-weight", name: "fetchWeightTrend", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "SELECT * FROM weight_logs WHERE user_id = $1 ORDER BY logged_at DESC LIMIT 30",
              params: ["$user.id"] },
      onSuccess: "SET $local.weightTrend = $result.rows"
    }},
    { id: "act-log-weight", name: "logWeight", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: "INSERT INTO weight_logs (user_id, weight_kg) VALUES ($1, $2) RETURNING *",
    ...
    { id: "act-fetch-weekly", name: "fetchWeeklyWorkouts", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT date_trunc('day', created_at) AS day, COUNT(*) AS count
    ...
    { id: "act-fetch-analytics", name: "fetchAnalytics", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT COUNT(*) AS total_workouts,
    ...
    { id: "act-fetch-cal-trend", name: "fetchCaloriesTrend", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT date_trunc('month', created_at) AS month, SUM(calories_burned) AS total
    ...
    { id: "act-fetch-today", name: "fetchTodayActivity", type: "fetch", config: {
      url: "/api/db/{projectId}", method: "POST",
      body: { sql: `SELECT COALESCE(SUM(steps),0) AS steps, COALESCE(SUM(calories),0) AS calories,
